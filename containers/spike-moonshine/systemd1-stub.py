#!/usr/bin/env python3
"""SPIKE: a stub `org.freedesktop.systemd1` on the session bus.
...
def open_stream(value, default):
    ...
    PS_STUB_STDIO overrides whatever the unit asked for. moonshine-bench builds
    its ApplicationConfig with the defaults, i.e. StandardOutput=null, which
    silences exactly the output a spike wants to read; `PS_STUB_STDIO=inherit`
    routes it to the stub's own stderr instead.
    """
