...
- `--device /dev/uinput` + `-v /dev/input:/dev/input`: sunshine creates its
  ...
  /dev/input.
- `--device /dev/ntsync`: in-kernel NT sync primitives for Proton instead of
  fsync/esync. Added only when the host node exists and is writable for this
  user (kernel 6.14+ with the `ntsync` module loaded); `--device` on a missing
  node would fail container creation. Which games use it is the Proton build's
  call (proton-cachyos 10.x: curated appid list plus `PROTON_USE_NTSYNC`; 11.x:
  device presence, `PROTON_NO_NTSYNC` to opt out; GE: `WINENTSYNC`), so the
  image exports no ntsync variable. Absent, Proton uses fsync.
...
- **Rootless input hotplug.** The kernel delivers no udev uevents into a
  rootless user namespace. The seat shim therefore fakes labwc's udev monitor
